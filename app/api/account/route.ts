...
export async function GET(request: Request) {
  ...
  const promo = await env.DB.prepare("SELECT player_email FROM promo_claims WHERE player_email = ?").bind(user.email).first();
  const response = Response.json({ displayName: player?.display_name ?? user.displayName, balance: player?.balance ?? 100, demo: user.demo, bonusClaimed: Boolean(claim), promoClaimed: Boolean(promo) });
