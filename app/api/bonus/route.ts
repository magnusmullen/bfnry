...
export async function POST(request: Request) {
  ...
  const claim = await env.DB.prepare("INSERT OR IGNORE INTO bonus_claims (player_email, created_at) VALUES (?, ?)").bind(user.email, now).run();
  const awarded = Number(claim.meta.changes ?? 0) > 0;
  if (awarded) await env.DB.prepare("UPDATE players SET balance = balance + 30, updated_at = ? WHERE email = ?").bind(now, user.email).run();
  const player = await env.DB.prepare("SELECT balance FROM players WHERE email = ?").bind(user.email).first<{ balance: number }>();
  const response = Response.json({ displayName: user.displayName, balance: player?.balance ?? 100, demo: user.demo, awarded, bonusClaimed: true });
