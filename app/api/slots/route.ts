...
export async function POST(request: Request) {
  ...
  const grid = Array.from({ length: 3 }, (_, row) => Array.from({ length: 5 }, (_, reel) => symbolFromRandom(random[row * 5 + reel]))) as SlotGrid;
  const wins = evaluateSlotGrid(grid, bet);
  const luckyBonus = evaluateLuckyBonus(grid, bet);
  const payout = wins.reduce((sum, win) => sum + win.payout, 0) + (luckyBonus?.payout ?? 0);
  ...
  const response = Response.json({ displayName: user.displayName, balance, demo: user.demo, grid, wins, luckyBonus, payout, delta, bet });
